      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-end mb-12">
            <div><h2 className="text-3xl font-bold text-slate-900">Trending Galleries</h2><p className="text-slate-500 mt-2">Find your event and start the magic.</p></div>
            <Link to="/features" className="hidden md:flex items-center text-indigo-600 font-bold hover:underline">How it works <ArrowRight size={16} className="ml-1" /></Link>
          </div>
          {recentEvents.length === 0 ? (
            <div className="text-center py-20 border-2 border-dashed border-slate-200 rounded-2xl bg-white"><Camera className="w-16 h-16 text-slate-300 mx-auto mb-4" /><p className="text-xl text-slate-500">No public events active right now.</p></div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">{recentEvents.map((event) => (
                <Link key={event.id} to={`/gallery/${event.id}`} className="group bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                  ...
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-2"><h3 className="font-bold text-xl text-slate-900 leading-tight group-hover:text-indigo-600 transition-colors">{event.name}</h3><span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-1 rounded-full">Active</span></div>
                    <p className="text-sm text-slate-500 mb-6">{event.date}</p>
                    <div className="w-full bg-slate-50 hover:bg-indigo-50 text-slate-700 hover:text-indigo-700 font-bold py-3 rounded-xl flex items-center justify-center transition-colors">View Photos <ArrowRight className="ml-2 w-4 h-4" /></div>
                  </div>
                </Link>
              ))}</div>
          )}
