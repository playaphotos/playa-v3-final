...
import { Camera, Upload as UploadIcon, CheckCircle, ArrowLeft, X, AlertTriangle } from 'lucide-react';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import { collection, addDoc, serverTimestamp } from 'firebase/firestore';
import { storage, db } from '../lib/firebase';

const MobileSnap: React.FC = () => {
  ...
  // 1. FORCE EVENT NAME (Prevents "Event Not Found")
  useEffect(() => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // C. Redirect to Wall after 3 seconds
      setTimeout(() => {
         setFile(null); 
         setPreview(null); 
         setStatus('idle');
         // Redirect user to the Live Wall to see their photo
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans flex flex-col">
      ...
      {/* DEBUG BANNER: ORANGE = SUCCESS */}
      <div className="bg-orange-500 text-white text-[10px] p-1 text-center font-mono font-bold tracking-widest uppercase animate-pulse">
         CACHE BYPASSED SUCCESS
      </div>
         ...
         <AnimatePresence>
            ...
            {status === 'idle' || status === 'uploading' || status === 'error' ? (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-md flex flex-col items-center">
                  ...
                  {!preview ? (
                     <>
                        ...
                        <h2 className="text-3xl font-bold text-center mb-4">Take a Snap</h2>
                        <div className="w-full grid gap-4">
                           <button onClick={() => fileInputRef.current?.click()} className="w-full py-5 bg-orange-600 text-white font-bold rounded-2xl shadow-lg shadow-orange-500/20 active:scale-95 transition-transform">
                     ...
                     <div className="w-full flex flex-col items-center">
                        ...
                        <div className="relative w-full aspect-[3/4] rounded-2xl overflow-hidden mb-8 border border-slate-700 shadow-2xl">
                           ...
                           <button onClick={() => { setFile(null); setPreview(null); }} className="absolute top-4 right-4 bg-black/50 text-white p-2 rounded-full hover:bg-black/70"><X size={20} /></button>
                        </div>
                        {/* SEND BUTTON */}
                        <button onClick={handleUpload} disabled={status === 'uploading'} className="w-full py-5 bg-emerald-500 text-white font-bold rounded-2xl hover:bg-emerald-400 flex items-center justify-center gap-3 text-lg disabled:opacity-50 shadow-lg shadow-emerald-500/20 active:scale-95 transition-transform">
                           {status === 'uploading' ? 'Sending...' : <><UploadIcon size={24} /> Send to Wall</>}
                        </button>
                     </div>
                  )}
                  {/* HIDDEN INPUT */}
               ...
               <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="text-center">
                  <div className="w-32 h-32 bg-emerald-500 rounded-full flex items-center justify-center mx-auto mb-8 shadow-2xl shadow-emerald-500/30">
                     <CheckCircle size={64} className="text-white" />
                  </div>
                  <h2 className="text-4xl font-bold mb-4">Sent!</h2>
                  <p className="text-slate-400">Look at the big screen...</p>
               </motion.div>
            )}
         </AnimatePresence>
      </div>
